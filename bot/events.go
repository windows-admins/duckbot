...
func handlePlusMinus(item string, operation string, s *discordgo.Session, m *discordgo.Message, user *discordgo.User) {
	if item == m.Author.ID {
		s.ChannelMessageSend(m.ChannelID, "Santa told me you get coal this year")
		return
	}
	...
	if user == nil {
		s.ChannelMessageSend(m.ChannelID, fmt.Sprintf("%[1]s has %[2]d present%[3]s", item, score, plural))
